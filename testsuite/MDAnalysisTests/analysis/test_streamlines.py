...
def test_streamplot_2D(univ):
    ...
    analysis = Streamlines(
        ...
        select="name PO4",
        grid_spacing=20,
        x_min=univ.atoms.positions[...,0].min(),
        x_max=univ.atoms.positions[...,0].max(),
        y_min=univ.atoms.positions[...,1].min(),
        y_max=univ.atoms.positions[...,1].max(),
        maximum_delta_magnitude=2.0,
    )
    analysis.run(start=1, stop=3)
